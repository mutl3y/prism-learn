"""Section-title aggregation reporting helpers."""
    ...
    *,
    ...
    if source == "reduced":
        ...
        for (
            ...
            normalized = str(normalized_title or "").strip() or normalize_style_heading(
                heading
            )
    ...
    for target, row_batch_id, captured_at_utc, raw_stats, raw_sections in rows:
        stats = coerce_json_document(raw_stats)
        if not stats:
